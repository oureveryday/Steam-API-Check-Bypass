...
BOOL APIENTRY DllMain(CONST HMODULE hModule, CONST DWORD fdwReason, CONST LPVOID lpvReserved)
{
    ...
    if (GetFileAttributesA("libraries.txt") == INVALID_FILE_ATTRIBUTES)
    {
        if (!LoadLibraryA("SteamAPICheckBypass.dll"))
        {
            MessageBoxA(NULL, "Failed to load SteamAPICheckBypass.dll", PROJECT_NAME, ErrBoxType);
        }
        return TRUE;
    }
    ...
    {
    	MessageBoxA(NULL, "Failed to open libraries.txt", PROJECT_NAME, ErrBoxType);
        return TRUE;
    }
    LoadLibraryA("SteamAPICheckBypass.dll");
